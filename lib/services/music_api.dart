...
class MusicApi {
  ...
  Future<List<Song>> searchSongs(
    ...
    if (kDebugMode) {
      debugPrint('[REIKO][search] keywords="$keywords"');
      debugPrint('[REIKO][search] raw type: ${raw.runtimeType}');
      if (raw is List) {
        debugPrint('[REIKO][search] list length: ${raw.length}');
      } else if (raw is Map) {
        debugPrint('[REIKO][search] map keys: ${raw.keys.toList()}');
      }
    }
...
void _debugLyricLog(String message) {
  ...
  debugPrint('[REIKO][lyrics] $message');
}
...
void _debugLyricContent(String label, String content) {
  ...
  debugPrint('[REIKO][lyrics] ==== end $label ====');
}
...
void _debugPlaylistLogObject(String label, Object? value) {
  ...
  final text = const JsonEncoder.withIndent('  ').convert(value);
  debugPrint('[REIKO][playlists] ==== $label ====');
  const chunkSize = 1800;
  ...
  debugPrint('[REIKO][playlists] ==== end $label ====');
}
...
void _debugArtistLogObject(String label, Object? value) {
  ...
  final text = const JsonEncoder.withIndent('  ').convert(value);
  debugPrint('[REIKO][artist] ==== $label ====');
  const chunkSize = 1800;
  for (var start = 0; start < text.length; start += chunkSize) {
    final end = (start + chunkSize).clamp(0, text.length);
    debugPrint(text.substring(start, end));
  }
  debugPrint('[REIKO][artist] ==== end $label ====');
}
